...
# Blueprint for MT3000 optimization/compute
compute_optimization_bp = Blueprint("mt3000_optimization_compute", __name__)


_REMOTE_WORK_DIR_TMPL = "/thfs3/home/{username}/MCJM-Toolkits/src/COMP"


@compute_optimization_bp.route("/mt3000/optimization/compute", methods=["POST"])
def submit_mt3000_optimization_compute():
    """Receive config.yaml, save locally, then upload to MT3000 remote COMP directory."""
    ...
    if not (lower.endswith(".yaml") or lower.endswith(".yml")):
        return jsonify({"error": "Unsupported file type (expected .yaml/.yml)"}), 400

    # Save under local uploads
    ...
    save_name = f"{unique_id}{ext or '.yaml'}"
    ...
    remote_path = f"{remote_dir}/config.yaml"
    ...
    return jsonify({
        "message": "config.yaml received and uploaded to MT3000",
