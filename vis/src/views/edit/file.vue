  ...
  <div class="edit-container">
    ...
    <el-card shadow="never" style="margin-top: 10px;">
          ...
          <el-row :gutter="10">
            <el-col :span="6">
              <el-form-item label="搜索:">
                <el-input v-model="search.query" placeholder="请输入文件名" />
              </el-form-item>
            </el-col>
            <el-col :span="6">
      ...
      <el-table :data="tableData" stripe>
        ...
        <el-table-column prop="server_filename" label="服务器文件名"/>
...
const getList = async () => {
  try {
    const res = await proxy.$request.get(import.meta.env.VITE_URL+'/files/', { params: search.value });
...
const saveData = async () => {
  try {
    if (form.value.id) {
      const res = await proxy.$request.put(import.meta.env.VITE_URL+'/files/' + form.value.id + '/', form.value);
      if (res.data.code === 200) {
        ElMessage.success({
      ...
      const res = await proxy.$request.post(import.meta.env.VITE_URL+'/files/', form.value);
      if (res.data.code === 200) {
        ElMessage.success({
          message:res.data.data,
          duration:1500
...
const deleteItem = async (id) => {
  try {
    const res = await proxy.$request.delete(import.meta.env.VITE_URL+'/files/' + id + '/');
    if (res.data.code === 200) {
      ElMessage.success({
          message:res.data.data,
          duration:1500
