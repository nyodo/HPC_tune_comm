  ...
  <div class="edit-container">
    ...
    <el-card shadow="never" style="margin-top: 10px;">
      ...
      <el-table :data="tableData" stripe>
        ...
        <el-table-column prop="analysis" label="建模分析" width="500" />
        <el-table-column prop="improve" label="调优代码" width="500" />
    ...
    <sDrawer v-model="visible" :title="form.id ? '编辑记录' : '添加记录'" size="35%" :close-on-click-modal="false">
      ...
      <el-form :model="form" label-width="100px" class="drawer-form">
        ...
        <el-form-item label="建模分析:">
          <el-input v-model="form.analysis" type="textarea" :rows="4" placeholder="请输入内容"></el-input>
        </el-form-item>
        <el-form-item label="调优代码:">
          <el-input v-model="form.improve" type="textarea" :rows="4" placeholder="请输入内容"></el-input>
        </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      tableData: [],
      totalNum: 100,
      // 提交给后端的搜索信息，后面可以加上用户id，判断用户id然后提供内容。管理员看所有的，用户看自己的。
      search: {
        pageNum: 1,
        pageSize: 10,
        query: "",
  ...
  methods: {
    ...
    async saveData() {
      if (this.form.id) {
        ...
        const res = await this.$request.put(import.meta.env.VITE_URL + '/dcus/' + this.form.id + '/', this.form)
        if (res.data.code === 200) {
        ...
        const res = await this.$request.post(import.meta.env.VITE_URL + '/dcus/', this.form)
        if (res.data.code === 200) {
          this.$message.success(res.data.data)
          this.getList()
          this.visible = false
    ...
    async deleteItem(id) {
      const res = await this.$request.delete(import.meta.env.VITE_URL + "/dcus/" + id + '/')
      if (res.data.code === 200) {
        this.$message.success(res.data.data)
        this.getList()
      }
