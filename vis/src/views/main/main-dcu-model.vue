    ...
    <sDrawer v-model="drawers.code.visible" title="上传代码" size="35%" :close-on-click-modal="false">
      <el-form :model="drawers.code.form" label-width="100px" class="drawer-form">
        ...
        <el-form-item v-if="drawers.code.uploadType === 'file'" label="文件:">
          <el-upload class="upload-demo" drag :action="uploadFileUrl" :limit="1" accept=".txt,.cu,.cpp"
...
const handleSubmit = async () => {
  ...
  try {
    ...
    const response = await axios.post(import.meta.env.VITE_URL + '/dcu_model', data);
    handleReset();
...
const handleEvaluate = async () => {
  ...
  try {
    ...
    const response = await axios.post(import.meta.env.VITE_URL + '/dcu_evaluate', data);
